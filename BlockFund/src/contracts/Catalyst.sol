...
contract Catalyst {
    ...
    function backProject(uint id) public payable returns (bool) {
        require(msg.value > 0 ether, "Amount must be valid number");
        require(projectExist[id], "Project not found");
        require(projects[id].status == statusEnum.OPEN, "Project no longer opened");
    ...
    function requestRefund(uint id) public returns (bool) {
        require(
            projects[id].status != statusEnum.REVERTED ||
            projects[id].status != statusEnum.DELETED,
            "Project not marked as revert or deleted"
        );
         
        require(projects[id].status == statusEnum.OPEN, "Project already closed");
        require(block.timestamp >= projects[id].expiresAt, "Deadline not passed");
    ...
    function payOutProject(uint id) public returns (bool) {
        ...
        require(address(this).balance >= projects[id].raised, "Insufficient Fund");
